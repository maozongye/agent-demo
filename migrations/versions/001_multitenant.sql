-- 001_multitenant (up)
-- Multi-tenant tables, session.tenant_id, knowledge_base, email_draft
...
-- Nullable initially for legacy rows; new sessions should set tenant_id
ALTER TABLE session ADD COLUMN IF NOT EXISTS tenant_id INTEGER REFERENCES tenant(id);
